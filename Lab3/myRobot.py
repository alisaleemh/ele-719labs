...
class myRobot(Robot):
    ...
    def inverse_kinematics(self, p_dot, theta):
        ...
        m1_inv = (1.0/wheel_radius)*(array([[sin(theta), -cos(theta), -L], [cos((pi/6)+theta), sin((pi/6)+theta), -L], [-cos((pi/6)-theta), sin((pi/6)-theta), -L]]))
        wheel_angular_velocities = dot(m1_inv,p_dot)
    ...
    def forward_kinematics(self, wheel_angular_velocities, theta):
        L = self._L
        wheel_radius = self._wheel_radius
        m1 = (wheel_radius)*(array(([[sin(theta), -cos(theta), -L], [cos((pi/6)+theta), sin((pi/6)+theta), -L], [-cos((pi/6)-theta), sin((pi/6)-theta), -L]])^-1))
